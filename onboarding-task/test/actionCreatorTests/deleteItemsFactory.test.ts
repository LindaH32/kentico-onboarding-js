...
describe('Correctly resolves deleteItem: ', () => {
  ...
  const items = [
    { Id: fakeFirstId, Text: 'text' },
    { Id: '1c353e0a-5481-4c31-bd2e-47e1baf84dbe', Text: 'panda' },
  ];
  const deleteSuccess = () => Promise.resolve({ json: () => Promise.resolve(items) });
  const deleteFailImmediately = () => Promise.reject(new Error('Item could not be deleted'));
  const deleteFail = () => Promise.resolve({ json: () => Promise.reject(new Error('Item could not be deleted')) });
  ...
  const deleteItem = (itemDelete: () => Promise<ResponseWithJson>) => deleteItemFactory({
